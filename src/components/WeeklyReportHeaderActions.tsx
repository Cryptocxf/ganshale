...
/** 顶栏「查看周报」「生成周报」（周选择器在 AppChrome 左侧） */
export function WeeklyReportHeaderActions() {
  const report = useWeeklyReportOptional()
  const [autoLabel, setAutoLabel] = useState<string | null>(() => formatWeeklyAutoTriggerLabel())

  useEffect(() => {
    const sync = () => setAutoLabel(formatWeeklyAutoTriggerLabel(loadReportScheduleSettings()))
  ...
  const { weekStart, streaming, toast, onGenerate, openHistoryModal } = report
  const weekKind = compareLocalCalendarWeek(weekStart)
  const isFuture = weekKind === 'future'
  const isPast = weekKind === 'past'
