import { Copy, Download, FileSpreadsheet, Trash2, X } from 'lucide-react'
...
import {
  ...
  removeDailyReportHistoryEntry,
  sortDailyReportHistoryByTimeAsc,
  type DailyReportHistoryEntry,
} from '../lib/dailyReportHistoryStore'
import { exportDailyReportHistoryExcel } from '../lib/dailyReportHistoryExport'
import { formatDatetimeZh, toYmdLocal } from '../lib/timeutil'
...
export function DailyReportHistoryModal({
  open,
  day,
  onClose,
  onCopyToast,
  ...
  const [entries, setEntries] = useState<DailyReportHistoryEntry[]>(() =>
    loadDailyReportHistory(day),
  )

  const sortedEntries = useMemo(
    () => sortDailyReportHistoryByTimeAsc(entries),
    [entries],
  )

  const reload = useCallback(() => {
    setEntries(loadDailyReportHistory(day))
  }, [day])
  ...
  const handleDelete = useCallback(
    (id: string) => {
      removeDailyReportHistoryEntry(day, id)
      reload()
    },
    [day, reload],
  )
  ...
  const handleExportExcel = useCallback(() => {
    if (sortedEntries.length === 0) {
      onCopyToast('暂无记录可导出')
      window.setTimeout(() => onCopyToast(null), 2000)
      return
    }
    try {
      exportDailyReportHistoryExcel(toYmdLocal(day), sortedEntries)
    } catch {
      onCopyToast('导出失败')
      window.setTimeout(() => onCopyToast(null), 2000)
    }
  }, [day, sortedEntries, onCopyToast])
  ...
  return (
    <DashboardModalRoot
      ...
      onClose={onClose}
      zIndex={95}
      labelledBy="daily-report-history-title"
    ...
    >
        ...
        >
          <DashboardSectionTitle id="daily-report-history-title" icon={FileSpreadsheet}>
            日报记录 · {ymd}
          ...
          <div className="flex shrink-0 items-center gap-1">
            <button
              ...
              className="gs-toolbar-btn gs-toolbar-btn--accent inline-flex items-center gap-1 disabled:cursor-not-allowed disabled:opacity-40"
            >
