...
import { formatWeekRangeLabel, toYmdLocal } from '../lib/timeutil'
...
export function WeeklyReportResultModal({
  open,
  weekStart,
  text,
  streaming,
  ...
  const handleExport = useCallback(() => {
    ...
    downloadTextFile(`周报-${toYmdLocal(weekStart)}.txt`, body)
    onCopyToast('已导出 TXT')
    window.setTimeout(() => onCopyToast(null), 2500)
  }, [text, weekStart, onCopyToast])
  ...
  return (
    <DashboardModalRoot
      ...
      zIndex={95}
      enterAnimation
      labelledBy="weekly-report-result-title"
    ...
    >
        ...
        >
          <DashboardSectionTitle id="weekly-report-result-title" icon={FileText}>
            周报 · {range}
        ...
        >
          {showSpinner ? (
            ...
            <p className="text-sm text-ganshale-muted">
              暂无周报内容。请点击「生成周报」，将根据本周日报与工作记录汇总生成。
            </p>
          )}
        </div>
    </DashboardModalRoot>
