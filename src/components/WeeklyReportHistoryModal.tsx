...
import {
  ...
  removeWeeklyReportHistoryEntry,
  sortWeeklyReportHistoryByTimeAsc,
  WEEKLY_REPORT_HISTORY_CHANGED_EVENT,
  type WeeklyReportHistoryEntry,
} from '../lib/weeklyReportHistoryStore'
import { formatDatetimeZh, formatWeekRangeLabel, toYmdLocal } from '../lib/timeutil'
...
export function WeeklyReportHistoryModal({
  open,
  weekStart,
  onClose,
}: {
  ...
  const [entries, setEntries] = useState<WeeklyReportHistoryEntry[]>(() =>
    loadWeeklyReportHistory(weekStart),
  )

  const sortedEntries = useMemo(
    () => sortWeeklyReportHistoryByTimeAsc(entries),
    [entries],
  )

  const reload = useCallback(() => {
    setEntries(loadWeeklyReportHistory(weekStart))
  }, [weekStart])
  ...
  const handleDelete = useCallback(
    (id: string) => {
      removeWeeklyReportHistoryEntry(weekStart, id)
      reload()
    },
    [weekStart, reload],
  )
  ...
  const weekKey = toYmdLocal(weekStart)
  ...
  return (
    <DashboardModalRoot
      ...
      onClose={onClose}
      zIndex={95}
      labelledBy="weekly-report-history-title"
    ...
    >
        ...
        >
          <DashboardSectionTitle id="weekly-report-history-title" icon={FileSpreadsheet}>
            周报记录 · {weekKey}
        ...
        <div className={[DASHBOARD_DETAIL_MODAL_BODY_CLASS, 'px-1 py-1 sm:px-2 sm:py-2'].join(' ')}>
          <p className="mb-2 px-2 text-[10px] text-ganshale-muted">{range}</p>
          {sortedEntries.length === 0 ? (
            <p className="px-2 py-6 text-center text-sm text-ganshale-muted">
              暂无周报记录。请点击「生成周报」生成后在此查看。
            </p>
          ) : (
